...
const state={
  settings:loadJson(STORAGE_KEY,defaults),
...
const ids=[
  'hero','overallDot','overallStatus','qualityScore','qualityLabel',
  'lastCheck','testDuration','runProgress','progressText','fullBtn',
  'quickBtn','shareBtn','settingsBtn','internetPill','internetMetric',
  'internetDetail','latencyPill','latencyMetric','latencyDetail',
  'latencyDelta','jitterPill','jitterMetric','lossPill','lossMetric',
  'lossDetail','downloadPill','downloadMetric','downloadDetail',
  'downloadDelta','uploadPill','uploadMetric','uploadDetail',
  'uploadDelta','networkPill','networkMetric','networkDetail',
  'batteryPill','batteryMetric','batteryBar','batteryDetail',
  'verdictPill','verdictSummary','gamingVerdict','callsVerdict',
  'streamVerdict','browseVerdict','deviceMetric','deviceGrid',
  'securityPill','securityScore','securitySummary','securityBar',
  'securityList','serviceList','serviceSummary','latencyChart',
  'downloadChart','uploadChart','latencyLatest','downloadLatest',
  'uploadLatest','settingsPanel','closeSettingsBtn','timeoutInput',
  'intervalInput','serviceEditors','addServiceBtn','resetBtn','saveBtn'
];
...
function loadJson(k,f){
  try{
    return JSON.parse(localStorage.getItem(k))??clone(f);
  }catch{
    return clone(f);
  }
}
...
function classForStatus(s){
  return s==='good'?'good':s==='bad'?'bad':s==='warn'?'warn':'';
}

function setPill(el,text,status=''){
  el.textContent=text;
  el.className=`pill ${classForStatus(status)}`.trim();
}
...
function osName(){
  ...
  if(
    ...
    (/Mac/i.test(p)&&navigator.maxTouchPoints>1)
  )return'iOS / iPadOS';
...
function detectDevice(){
  ...
  const platform=
    ...
    /iPad/i.test(ua)?'iPad':
    mobile?'Mobile device':'Desktop / tablet';

  els.deviceMetric.textContent=platform;

  const rows=[
    ['OS / platform',osName()],
    ['Browser',browserName()],
    ['CPU',`${navigator.hardwareConcurrency||'Unknown'} logical cores`],
    ['Memory',navigator.deviceMemory?`${navigator.deviceMemory} GB approx.`:'Not exposed'],
  ...
  els.deviceGrid.innerHTML=rows.map(
    ([k,v])=>`<div class="kv"><span>${k}</span><span>${v}</span></div>`
  ).join('');
}
...
function updateNetworkInfo(){
  ...
  if(!c){
    els.networkMetric.textContent=navigator.onLine?'Connected':'Offline';

    els.networkDetail.textContent=
      'Detailed Wi-Fi/cellular information is not exposed by this browser.';

    setPill(
      els.networkPill,
      'Limited',
      navigator.onLine?'warn':'bad'
    );
  ...
  if(c.downlink)bits.push(`${c.downlink} Mbps estimated`);
  if(Number.isFinite(c.rtt))bits.push(`${c.rtt} ms RTT estimated`);
  if(c.saveData)bits.push('Data Saver on');

  els.networkMetric.textContent=String(type).toUpperCase();

  els.networkDetail.textContent=
    bits.join(' · ')||'Connection API available.';

  setPill(
    els.networkPill,
    c.saveData?'Data Saver':'Detected',
    c.saveData?'warn':'good'
  );
}
...
async function updateBattery(){
  if(!navigator.getBattery){
    els.batteryMetric.textContent='Unavailable';

    els.batteryDetail.textContent=
      'This browser does not expose battery status.';

    setPill(els.batteryPill,'Restricted','warn');
  ...
  try{
    ...
    const render=()=>{
      ...
      els.batteryMetric.textContent=`${pct}%`;
      els.batteryBar.style.width=`${pct}%`;
      els.batteryDetail.textContent=b.charging?'Charging':'On battery';

      setPill(
        els.batteryPill,
        b.charging?'Charging':pct<20?'Low':'Normal',
        b.charging||pct>=20?'good':'warn'
      );
    };

    render();

    b.addEventListener('levelchange',render,{passive:true});
    b.addEventListener('chargingchange',render,{passive:true});

  }catch{
    els.batteryMetric.textContent='Unavailable';
    setPill(els.batteryPill,'Error','warn');
  }
}

/*
 * Browser-visible security assessment only.
 *
 * Removed:
 * - Wi-Fi encryption
 * - Router security
 ...
 *
 * A normal browser page cannot reliably inspect those properties.
 */
function renderSecurityAssessment(){
  const checks=[];

  const add=(name,detail,status,points,earned)=>
    checks.push({name,detail,status,points,earned});
  ...
  const https=protocol==='https:'||localhost;

  add(
    'Encrypted page transport',
    https
      ?(
        protocol==='https:'
          ?'Dashboard loaded over HTTPS.'
          :'Localhost trusted development context.'
      )
      :`Loaded over ${protocol||'unknown transport'}; HTTPS protection is not confirmed.`,
    https?'good':'bad',
    30,
    https?30:0
  );
  ...
  add(
    ...
    secure
      ?'Sensitive browser APIs are running in a secure context.'
  ...
  const insecure=
    resources.filter(r=>/^http:\/\//i.test(r.name));

  const mixedSafe=insecure.length===0;

  add(
    'Mixed-content exposure',
    mixedSafe
      ?'No HTTP subresources observed.'
      :`${insecure.length} HTTP subresource(s) observed.`,
    mixedSafe?'good':'bad',
    15,
    mixedSafe?15:0
  );

  const eps=state.settings.services||[];

  const badEps=
    eps.filter(s=>!/^https:\/\//i.test(s.url));

  const epsSafe=
    eps.length>0&&badEps.length===0;

  add(
    'Configured endpoint transport',
    epsSafe
      ?`All ${eps.length} endpoints use HTTPS.`
      :badEps.length
        ?`${badEps.length} endpoint(s) do not use HTTPS.`
  ...
  add(
    'Page embedding',
    embedded
      ?'Dashboard is embedded in another page/frame.'
      :'Dashboard is running as a top-level page.',
    embedded?'warn':'good',
    10,
    embedded?4:10
  );
  ...
  add(
    'Web cryptography support',
    crypto
      ?'Modern Web Crypto API available.'
      :'Web Crypto is unavailable or restricted.',
    crypto?'good':'warn',
    10,
    crypto?10:4
  );
  ...
  const total=
    checks.reduce((n,c)=>n+c.points,0);

  const earned=
    checks.reduce((n,c)=>n+c.earned,0);

  const score=
    clamp(Math.round(earned/total*100),0,100);

  let label='Low visible risk';
  let status='good';

  if(score<55){
    label='High visible risk';
    status='bad';
  }else if(score<80){
  ...
  els.securityScore.innerHTML=
    `${score}<small>/100</small>`;

  els.securityBar.style.width=
    `${score}%`;

  els.securitySummary.textContent=
    `${label}. This score covers only signals a web page can observe.`;
  ...
  checks.forEach(c=>{
    ...
    row.innerHTML=`
      <span class="security-icon ${c.status}"></span>
      <div>
        <div class="security-name"></div>
        <div class="security-detail"></div>
      </div>
      <div class="security-weight"></div>
    `;

    row.querySelector('.security-name').textContent=
      c.name;

    row.querySelector('.security-detail').textContent=
      c.detail;

    row.querySelector('.security-weight').textContent=
      `${c.earned}/${c.points}`;
...
function renderServices(results=null){
  ...
  state.settings.services.forEach((s,i)=>{
    ...
    const statusClass=
      r
        ?classForStatus(r.ok?'good':r.timeout?'warn':'bad')
        :'';
    ...
    item.innerHTML=`
      <span class="service-light ${statusClass}"></span>
      <div style="min-width:0">
        <div class="service-name"></div>
        <div class="service-url"></div>
      </div>
      <div class="service-result">${text}</div>
    `;

    item.querySelector('.service-name').textContent=
      s.name||`Endpoint ${i+1}`;

    item.querySelector('.service-url').textContent=
      s.url;

    els.serviceList.appendChild(item);
  });
}
...
async function timedFetch(url,timeoutMs){
  ...
  try{
    ...
    await fetch(
      `${url}${sep}_status_check=${Date.now()}_${Math.random()}`,
      {
        method:'GET',
        mode:'no-cors',
        cache:'no-store',
...
function latencyStatus(ms){
  if(!Number.isFinite(ms))return['Unknown','warn'];
  if(ms<80)return['Excellent','good'];
  if(ms<180)return['Good','good'];
  if(ms<350)return['Fair','warn'];
...
function jitterStatus(ms){
  if(!Number.isFinite(ms))return['Unknown','warn'];
  if(ms<10)return['Excellent','good'];
  if(ms<30)return['Good','good'];
  if(ms<60)return['Fair','warn'];

  return['Poor','bad'];
}

function lossStatus(p){
  if(!Number.isFinite(p))return['Unknown','warn'];
  if(p===0)return['Excellent','good'];
  if(p<=5)return['Good','good'];
  if(p<=15)return['High','warn'];

  return['Severe','bad'];
}

function speedStatus(m){
  if(!Number.isFinite(m))return['Unavailable','warn'];
  if(m>=100)return['Excellent','good'];
  if(m>=25)return['Fast','good'];
  if(m>=5)return['Usable','warn'];

  return['Slow','bad'];
}

async function runProbeRounds(rounds){
  const samples=[];

  const endpointSummary=
    state.settings.services.map(()=>({
      ok:0,
      total:0,
      last:null
    }));
  ...
  for(let r=0;r<rounds;r++){
    updateProgress(
      8+Math.round((r/rounds)*24),
      `Measuring latency and stability · sample ${r+1} of ${rounds}`
    );

    const results=
      await Promise.all(
        state.settings.services.map(
          s=>timedFetch(s.url,state.settings.timeoutMs)
        )
      );

    results.forEach((x,i)=>{
      endpointSummary[i].total++;
      endpointSummary[i].last=x;

      if(x.ok){
        endpointSummary[i].ok++;
        samples.push(x.ms);
      }
    });
  ...
  const total=
    endpointSummary.reduce((n,x)=>n+x.total,0);

  const ok=
    endpointSummary.reduce((n,x)=>n+x.ok,0);
...
async function downloadTransfer(bytes){
  const c=new AbortController();
...
async function uploadTransfer(bytes){
  ...
  try{
    await fetch(
      `https://speed.cloudflare.com/__up?_=${Date.now()}`,
      {
        method:'POST',
        body:payload,
        mode:'no-cors',
...
function adaptiveDownloadBytes(first,full){
  if(!full)return 2_000_000;
  if(first>=150)return 25_000_000;
  if(first>=60)return 10_000_000;
  if(first>=20)return 5_000_000;

  return 2_000_000;
}

function adaptiveUploadBytes(first,full){
  if(!full)return 1_000_000;
  if(first>=80)return 10_000_000;
  if(first>=25)return 5_000_000;
  if(first>=8)return 2_000_000;
...
async function runSpeedTests(full){
  ...
  let download=NaN;
  let upload=NaN;
  let downBytes=2_000_000;
  let upBytes=1_000_000;
  ...
  try{
    ...
    const first=
      await downloadTransfer(2_000_000);

    downBytes=
      adaptiveDownloadBytes(first,full);

    download=
      downBytes===2_000_000
        ?first
        :await downloadTransfer(downBytes);

    els.downloadMetric.textContent=
      `${download.toFixed(download>=100?0:1)} Mbps`;

    const [l,c]=speedStatus(download);
    ...
    els.downloadDetail.textContent=
      `Measured using ${(downBytes/1e6).toFixed(0)} MB Cloudflare download.`;
    ...
    els.downloadDetail.textContent=
      `Download test failed${e?.name==='AbortError'?' (timed out)':''}.`;
  }
  ...
  try{
    ...
    const first=
      await uploadTransfer(1_000_000);

    upBytes=
      adaptiveUploadBytes(first,full);

    upload=
      upBytes===1_000_000
        ?first
        :await uploadTransfer(upBytes);

    els.uploadMetric.textContent=
      `${upload.toFixed(upload>=100?0:1)} Mbps`;

    const [l,c]=speedStatus(upload);
    ...
    els.uploadDetail.textContent=
      `Measured using ${(upBytes/1e6).toFixed(0)} MB Cloudflare upload.`;
    ...
    els.uploadDetail.textContent=
      `Upload test failed${e?.name==='AbortError'?' (timed out)':''}.`;
  }
...
function scoreLabel(score){
  if(score>=90)return['Excellent','good'];
  if(score>=78)return['Good','good'];
  if(score>=60)return['Fair','warn'];

  return['Poor','bad'];
}

function classifyUseCase(type,r){
  if(!r.internetOk){
    return['POOR','bad'];
  }
  ...
  const lat=r.latency;
  const jit=r.jitter;
  const down=r.download;
  const up=r.upload;

  if(type==='gaming'){
    if(loss<=2&&lat<80&&jit<20)
      return['EXCELLENT','good'];

    if(loss<=5&&lat<160&&jit<40)
      return['GOOD','good'];
...
function renderVerdict(r){
  ...
  els.verdictSummary.textContent=
    s>=90
      ?'Fast, stable connection with strong results across the measured signals.'
      :s>=78
        ?'Connection is healthy for most everyday workloads.'
        :s>=60
          ?'Connection is usable, but one or more measurements may affect demanding tasks.'
          :'Connection quality is poor or unstable; review the measurements below.';

  [
    ['gamingVerdict','gaming'],
    ['callsVerdict','calls'],
    ['streamVerdict','stream'],
    ['browseVerdict','browse']
  ].forEach(([id,type])=>{
    const [v,c]=
      classifyUseCase(type,r);
...
function updateDelta(
  ...
  el.textContent=
    `${pct>=0?'▲':'▼'} ${Math.abs(pct).toFixed(0)}% vs previous check`;
...
async function runChecks(full=true){
  ...
  els.overallDot.className=
    'overall-dot warn';

  els.hero.style.setProperty(
    '--status-glow',
    'rgba(255,176,32,.16)'
  );
  ...
  updateProgress(
    4,
    'Checking browser and network state…'
  );
  ...
  updateProgress(
    82,
    'Calculating connection quality…'
  );

  const endpointCount=
    state.settings.services.length;

  const internetOk=
    browserOnline&&probe.successes>0;

  const partial=
    internetOk&&probe.successes<probe.total;

  els.internetMetric.textContent=
    internetOk?'Online':'Offline';

  els.internetDetail.textContent=
    internetOk
      ?`${probe.successes} of ${probe.total} diagnostic requests responded across ${endpointCount} endpoint(s).`
      :'No diagnostic request completed successfully.';

  setPill(
    els.internetPill,
    internetOk?'Connected':'No access',
    internetOk
      ?(partial?'warn':'good')
      :'bad'
  );

  els.latencyMetric.textContent=
    Number.isFinite(probe.avg)
      ?`${Math.round(probe.avg)} ms`
      :'— ms';

  const [ll,lc]=
    latencyStatus(probe.avg);
  ...
  els.latencyDetail.textContent=
    Number.isFinite(probe.avg)
      ?`Average of ${probe.successes} successful request samples.`
      :'Latency could not be measured.';

  els.jitterMetric.textContent=
    Number.isFinite(probe.jitter)
      ?`${Math.round(probe.jitter)} ms`
      :'— ms';

  const [jl,jc]=
    jitterStatus(probe.jitter);
  ...
  els.lossMetric.textContent=
    `${Math.round(probe.loss)}%`;

  const [pl,pc]=
    lossStatus(probe.loss);
  ...
  els.lossDetail.textContent=
    `${probe.total-probe.successes} failed of ${probe.total} application-layer requests.`;

  setPill(
    els.serviceSummary,
    `${probe.successes}/${probe.total} responses`,
    probe.loss===0
      ?'good'
      :probe.loss<=15
        ?'warn'
  ...
  if(
    Number.isFinite(probe.avg)||
    Number.isFinite(speed.download)||
    Number.isFinite(speed.upload)
  ){
    state.history.push(result);
  ...
  els.lastCheck.textContent=
    `Last checked: ${nowLabel()}`;

  els.testDuration.textContent=
...
function drawAllCharts(){
  ...
  els.latencyLatest.textContent=
    Number.isFinite(last?.latency)
      ?`${Math.round(last.latency)} ms`
      :'— ms';

  els.downloadLatest.textContent=
    Number.isFinite(last?.download)
      ?`${last.download.toFixed(1)} Mbps`
      :'— Mbps';

  els.uploadLatest.textContent=
    Number.isFinite(last?.upload)
      ?`${last.upload.toFixed(1)} Mbps`
      :'— Mbps';
}
...
function buildReport(){
  ...
  const lines=[
    'NETVITALS REPORT',
    `Generated: ${nowLabel()}`,
    `Overall: ${els.overallStatus.textContent}`,
    `Quality: ${r?`${r.score}/100`:'Not checked'}`,
    `Internet: ${els.internetMetric.textContent}`,
    `Latency: ${els.latencyMetric.textContent}`,
    `Jitter: ${els.jitterMetric.textContent}`,
    `Request loss: ${els.lossMetric.textContent}`,
    `Download: ${els.downloadMetric.textContent}`,
    `Upload: ${els.uploadMetric.textContent}`,
    `Network: ${c?.effectiveType||c?.type||'Not exposed'}`,
    `Battery: ${els.batteryMetric.textContent}`,
    `Device: ${els.deviceMetric.textContent}`,
    `Security signals: ${
      state.securityResult
        ?`${state.securityResult.score}/100 — ${state.securityResult.label}`
        :'Not checked'
    }`,
    '',
    'USE CASES',
    `Gaming: ${els.gamingVerdict.textContent}`,
    `Video calls: ${els.callsVerdict.textContent}`,
    `Streaming: ${els.streamVerdict.textContent}`,
    `Browsing: ${els.browseVerdict.textContent}`,
    '',
    'ENDPOINTS'
  ];
  ...
  lines.push(
    '',
    'Note: Request-loss is an application-layer approximation. Security scoring covers browser-visible signals only.'
  );
...
function openSettings(){
  els.timeoutInput.value=
    state.settings.timeoutMs;

  els.intervalInput.value=
    state.settings.intervalSec;
...
function renderServiceEditors(services){
  els.serviceEditors.innerHTML='';
  ...
  services.forEach(s=>{
    ...
    row.innerHTML=`
      ...
      <label>
        HTTPS URL
        <input class="edit-url" type="url" value="">
      </label>

      <button
        class="danger-button remove-service"
    ...
    row.querySelector('.edit-url').value=
      s.url;
    ...
    els.serviceEditors.appendChild(
      row
    );
  });
}
...
function collectServices(){
  return[
    ...els.serviceEditors.querySelectorAll(
      '.service-editor'
    )
  ]
    .map((row,i)=>({
...
function scheduleAutoRefresh(){
  ...
  if(state.settings.intervalSec>0){
...
els.fullBtn.addEventListener(
  'click',
  ()=>runChecks(true)
);

els.quickBtn.addEventListener(
  'click',
  ()=>runChecks(false)
);
...
els.addServiceBtn.addEventListener(
  'click',
  ()=>{
    const current=
      collectServices();
    ...
    state.settings={
      ...
      intervalSec:
        Math.min(
          ...
          Math.max(
            0,
            Number(els.intervalInput.value)||0
          )
        ),
...
drawAllCharts();
scheduleAutoRefresh();
>>>>>>> 927fd10c5fac616b389d6607382e0c286cb0a1c6
